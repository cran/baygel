...
//' @export
// [[Rcpp::export]]
List blockBAGENI(arma::mat X, int burnin, int iterations, bool verbose = true, double r = 1e-3, double s = 1e-2, double a = 1e-3, double b = 1e-1){
 ...
 for (int iter=0; iter<totIter; iter++){
   ...
   for (int i=0; i<low_tri_tau_vals.size(); i++){
     ...
     double ridge_sig2 = 1.0/rrgamma(1, a_post, 1/b_post)(0);
     // lasso components
     double r_post = 1.0 + r;
     ...
     low_tri_tau_vals(i)= (scale_param + ridge_sig2)/(scale_param*ridge_sig2);
   }
